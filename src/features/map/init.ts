...
const onMapInit = sample({
  source: guard($countriesGeometryData, { filter: Boolean }),
  clock: changeMap,
});
...
$map.watch(
  ...
  (map, countriesGeometry: FeatureCollection) => {
    addCountriesToMap(map, countriesGeometry);
  }
);

$map.watch(onMapInit, (map, countriesGeometry) => {
  addCountriesToMap(map, countriesGeometry);
});
